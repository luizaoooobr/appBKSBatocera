...
app.post('/download', function (req, rest) {
  ...
  var exec3 = require('child_process').execsync;
   ...
   exec('ls /userdata/system/logs/*_download.json | wc -l ', (error, stdout, stderr) => {
       ...
       exec2('ls /userdata/system/logs/*_concluido.json | wc -l ', (error, stdout, stderr) => {  
              if (error) {
                 console.log(`error: ${error.message}`);
                 //return;
              }
            ...
            //carrega a base de download e concluidos
            exec('/usr/recalbox-manager2/config/sistemasdownload/scripts/downloads.sh', (err, stdout, stderr) => {
                if (err) {
                  console.error(err);
                  return;
                }
