...
app.post('/download', function (req, rest) {

  var exec = require('child_process').exec;
    
   exec('ls /userdata/system/logs/*_download.json | wc -l ', (error, stdout, stderr) => {
        if (error) {
        ...
        if ( temDown  != 0 ) {
            ...
            rest.send(`
                ...
                <body>
                    <form name="myform" action="/download1" method="POST" >
                     </form>
                    <script type="text/javascript">
                       document.myform.submit();
                     </script>
...
app.post('/download1', function (req, rest) {
   
    //var exec44 = require('child_process').execSync;
    var exec44 = require('child_process').exec;
    console.log("carrega a base de download e concluidos");
    exec44('/usr/recalbox-manager2/config/sistemasdownload/scripts/downloads.sh', (err, stdout, stderr) => {
        if (err) {
            console.error(err);
            rest.sendfile(_path.join(__dirname+'/romdown.html')); 
        }else{
            console.log("carrega a base de download");
            console.log(stdout);
            console.log(stderr);
            ...
            rest.send(`
                ...
                <body>
                    ...
                    <form name="myform" action="/download3" method="POST" >
...
app.post('/download3', function (req, rest) {
    console.log("Entrei no download3");
    const fs = require('fs')
  
    console.log("Carregando arquivos de download com os jogo para download")
    var caminho = "/userdata/system/logs/downloads.json";
    
    fs.readFile(caminho, 'utf8', (err, fileContents) => {
        if (err) {
            console.error(err)
            console.log("passe9 aqui");
            rest.sendfile(_path.join(__dirname+'/romdown.html'));
        }
        try {
            const data = JSON.parse(fileContents)
            ...
            var i;
            for (i = 0; i < data.length; i++) { 
                 console.log("Data : " + data[i].arquivo); 
            };
            ...
            rest.send(`
                ...
                <body>
                    ...
                    <form name="myform" action="/download4" method="POST" >
                        <input type="hidden" id="data" name="data" value="`+ data.arquivo +`" />
                     </form>
                    <script type="text/javascript">
                       document.myform.submit();
                     </script> 
...
app.post('/download4', function (req, rest) {
    console.log("/download4");
    const data2 = req.body;
    console.log("Arquivo = " + req.body[0].arquivo);   
    var i;
    for (i = 0; i < data2.length; i++) { 
        console.log("Data Recebida : " + data2[i].arquivo);  
    
    };
    
    console.log("sai do /download4");
    rest.sendfile(_path.join(__dirname+'/romdown.html'));
    ...
    console.log("carregando a tela dos arquivos de download")
                ...
                try {
                   ...
                   //console.log(data[0].fields.id);
                    ...
                    for (i = 0; i < data.length; i++) { 
                        ...
                        //Donwload ativos
                        //caminho2 = "/userdata/system/logs/"+data[i].arquivo;
                        caminho2 = data[i].arquivo;
                        console.log("caminho2: "+caminho2);
                        var fs2 = require('fs')
                        fs2.readFile(caminho2, 'utf8', (err, fileContents) => { 
                            if (err) {
                                console.error(err)
                                return
                            }
                            try { 
                                var data2 = JSON.parse(fileContents);
                                console.log("data2: "+data2[i].game); 
                            ...
                            //var data2 = retornaJason(caminho2);
                            
                            //console.log("data2: "+data2[i].nome); 
